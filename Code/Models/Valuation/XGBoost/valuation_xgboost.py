...
X_train, y_train = training_df[train_cols].drop(['Attendance', 'Valuation', 'Market', 'Year'], axis=1), training_df[['Valuation']]
X_test, y_test = test_df[test_cols].drop(['Attendance', 'Valuation', 'Market', 'Year'], axis=1), test_df[['Valuation']]
...
feature_model.fit(full_data[train_cols].drop(['Attendance', 'Valuation', 'Market', 'Year'], axis=1), full_data[["Valuation"]])
...
plt.title("Valuation XGB Model Feature Importance")
plt.savefig(f"{ROOT_PATH}/Visualizations/ValuationXGBFeatureImportance.png")
...
final_model.save_model("ValuationXGBoostModel.json")

# Write file to save stats
file = open('ValuationXGBEval.txt', 'w')
